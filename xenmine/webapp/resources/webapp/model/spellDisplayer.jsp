...
<c:choose>
  <c:when test="${empty(flybaseResults)}">
    <h3 class="goog gray">Expression by Developmental Stage</h3>
    ...
    <script type="text/javascript">
      function drawChart() {
          ...
          function howExpressed(text, range, data, n) {
              ...
              var _i, _len;
              for (_i = 0, _len = range.length; _i < _len; _i++) {
                data.setValue(n, _i + 1, (level == range[_i]["text"]) ? _i : 0);
                var display = range[_i]["range"]+":  "+score;
                data.setFormattedValue(n, _i + 1, display );
              }
              return data;
          }
