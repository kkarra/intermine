...
public class CreateFlankingRegions
{
    ...
    public CreateFlankingRegions(ObjectStoreWriter osw) {
        ...
        try {
            ...
            throw new RuntimeException(
                    "unable to fetch YeastMine DataSource object", e);
        }
    }
    ...
    public void createFlankingFeatures() throws ObjectStoreException {
        ...
        while (resIter.hasNext()) {
            ...
            if (PostProcessUtil.isInstance(model, gene, "NotInSystematicSequenceOfS288C")){
            	continue;
            }
            Location loc = (Location) rr.get(2);
            System.out.println("gene.."+gene+"   chr.."+ chrId);
            ...
            for (String direction : directions) {
                for (boolean includeGene : includeGenes) {
                    ...
                    if ("upstream".equals(direction) && "1".equals(strand)) {
                    	...
                    	start = geneEnd + (int) Math.round(distance * 1000);
                    	end = geneStart - (int) Math.round(distance * 1000);                    
                    }

                    // if the region hangs off the start or end of a chromosome set it to finish
                    // at the end of the chromosome
                    int d = Math.min(start,chr.getLength().intValue());
                    location.setStart(new Integer(d));
                    ...
                    System.out.println("GFR length info..start " + location.getStart().intValue() + "   end is ..." + location.getEnd().intValue());
