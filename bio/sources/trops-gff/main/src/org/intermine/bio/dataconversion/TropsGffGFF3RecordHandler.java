...
/**
 * A converter/retriever for the TropsGff dataset via GFF files.
 */

public class TropsGffGFF3RecordHandler extends GFF3RecordHandler
{

	/**
	 * Create a new TropsGffGFF3RecordHandler for the given data model.
	 * @param model the model for which items will be created
	 */
	public TropsGffGFF3RecordHandler (Model model) {
		super(model);
	...
	public void process(GFF3Record record) {
		...
		if ( "Gene".equals(clsName) ) {
			...
			String pacid = (String) ((List<?>) record.getAttributes().get("pacid")).get(0);
		
			if(pacid != null){
				feature.setAttribute("pacid", pacid);
			}
		}*/

		/* if ( "Gene".equals(clsName) ||  "MRNA".equals(clsName) ) {

        	System.out.println("gene class name .." + clsName);
