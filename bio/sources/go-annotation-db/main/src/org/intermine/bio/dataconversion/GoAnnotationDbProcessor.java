...
public class GoAnnotationDbProcessor {
	...
	protected ResultSet getGOAnnotations(Connection connection) throws SQLException {		
		...
		/*String query = "select sgdid, gene_name, ga.go_qualifier, goid, pmid, ra.display_name as sgdrefid, ea.display_name as evidence_code,"
				...
				+ "array_to_string(array_agg(ge.dbxref_id), ';') as annotext, "
				+ "array_to_string(array_agg(r.display_name), ';') as annotextension, " 
				+ "array_to_string(array_agg(gse.dbxref_id), ';') as withText "
				...
				+ " where db.dbentity_id in (1266463) and length(ea.display_name) <= 3 "
				+ "group by  sgdid, gene_name, ga.go_qualifier, goid, pmid, ra.display_name, ea.display_name, taxid, s.display_name, ga.annotation_type, gse.group_id"
		...
		String query = "select sgdid, gene_name, ga.go_qualifier, goid, pmid, ra.display_name as sgdrefid, ea.display_name as evidence_code, "
				...
				+ "left join nex.ro r on ge.ro_id = r.ro_id "
				+ "left join nex.gosupportingevidence gse on ga.annotation_id = gse.annotation_id "
				+ " where db.dbentity_id in (1266463, 1282786) and length(ea.display_name) <= 3 "
				...
				+ " order by sgdid, goid, pmid, gse.group_id"; //gse.dbxref_id,
