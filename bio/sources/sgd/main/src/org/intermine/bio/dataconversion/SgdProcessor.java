...
public class SgdProcessor
{
	...
	protected ResultSet getPhenotypeResults(Connection connection)
		...
		String query = "  select db.dbentity_id, pa.annotation_id, pac.group_id, p.display_name as phenotype, t.format_name as strain_name, pa.details,"
				...
				+ " array_to_string(array_agg(coalesce(condition_class, ' ')), '; ') as condclass,"
				+ " array_to_string(array_agg(coalesce(condition_name, ' ')), '; ') as condname , array_to_string(array_agg(coalesce(condition_value, ' ')), '; ') as condvalue,"
				+ " array_to_string(array_agg(coalesce(condition_unit, ' ')), '; ') as condunit"
				...
				//+ " where db.dbentity_id in (1269135,1286375)"
